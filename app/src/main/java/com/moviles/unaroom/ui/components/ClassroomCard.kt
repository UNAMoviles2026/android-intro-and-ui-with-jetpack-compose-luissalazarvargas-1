...
fun ClassroomCard(classroom: Classroom, modifier: Modifier = Modifier) {
    Card(
        ...
        Column(modifier = Modifier.padding(horizontal = 16.dp, vertical = 18.dp),
            ...
            ClassroomInfoRow(
                icon = { Icon(Icons.Outlined.People, null, Modifier.size(18.dp), AppIconTint) },
                text = "Capacity: ${classroom.capacity}"
            )
            ClassroomInfoRow(
                icon = { Icon(Icons.Outlined.LocationOn, null, Modifier.size(18.dp), AppIconTint) },
...
@Composable
private fun ClassroomInfoRow(icon: @Composable () -> Unit, text: String) {
    Row(verticalAlignment = Alignment.CenterVertically,
        horizontalArrangement = Arrangement.spacedBy(10.dp)) {
        icon()
        Text(text, color = AppSecondaryText, style = MaterialTheme.typography.bodyLarge)
    }
}
